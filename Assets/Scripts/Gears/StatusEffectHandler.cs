...
public class StatusEffectHandler : MonoSingleton<StatusEffectHandler>
{
    ...
    public void ApplyStatusEffect(GearTarget target, IStatusEffect statusEffect, uint amplitude)
    {
    ...
    {
        return Mathf.FloorToInt(Mathf.Max(0,Mathf.Log(amplitude, 4))) + 1;
    }
    public static float GetEffectLevelRaw(float amplitude)
    {
        return Mathf.Max(0,Mathf.Log(amplitude, 4)) + 1;
    }
}
